...
# Mail to user when Signed up as super admin
...
# Invitation for internal user
def sendEmailInvitationToUser(request, newUser, password):
    current_site = get_current_site(request)
    email_subject = 'Invitation from ' + str(request.user)
    message = render_to_string('users/user_login.html', {
        'user': newUser,
        'domain': current_site.domain,
        'password': password,
    })
